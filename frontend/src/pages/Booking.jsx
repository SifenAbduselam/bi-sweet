...
export default function Booking() {
  ...
  function validateForm() {
    ...
    if (!/^09\d{8}$/.test(form.phone_number)) {
      alert("Please enter Ethiopian phone number (09xxxxxxxx)");
      return false;
    }
  ...
  async function handleSubmit(e) {
    ...
    // Removed the isOrderingOpen() check so orders can be placed anytime
    if (!validateForm()) {
      return;
    }
    ...
    <section className="min-h-screen bg-gradient-to-br from-[#FFFBF7] via-[#FFF5F1] to-[#FFE8E0] py-32 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {/* Ordering Hours Label (Kept for information only) */}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-0 py-4 border-b border-gray-300 focus:outline-none focus:border-[#8B4A5A] transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                value={form.dessert}
                onChange={handleChange}
                className="w-full py-4 border-b border-gray-300 bg-transparent focus:outline-none focus:border-[#8B4A5A] transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full py-4 border-b border-gray-300 focus:outline-none focus:border-[#8B4A5A] transition-colors"
                />
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows="4"
                className="w-full border border-gray-300 p-3 rounded-md focus:outline-none focus:border-[#8B4A5A] transition-colors"
              />
            </div>

            <button
              disabled={loading}
              className="w-full bg-[#8B4A5A] hover:bg-[#723C4B] text-white py-4 uppercase tracking-widest font-semibold rounded-md transition-all hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? "Submitting..." : "Confirm Booking / ትዕዛዝ ያረጋግጡ"}
            </button>
          </form>
        </div>
      </div>
